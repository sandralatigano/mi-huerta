...
/**
 * Archivo: index.php
 * Descripción: Panel principal del sistema que muestra el listado de plantas activas, su estado y opciones de gestión rápida.
...
// --- CONFIGURACIÓN DE ORDENAMIENTO (Blindaje con Lista Blanca) ---
...
// --- CONSULTA PARA PAGINADO (Blindaje SQLi) ---
$total_query = "SELECT COUNT(*) as total FROM plantas WHERE nombre LIKE ? OR variedad LIKE ?";
...
// --- CONSULTA DE DATOS (Blindaje SQLi) ---
$query_datos = "SELECT p.*, e.nombre_epoca, t.nombre_tipo, b.nombre_bancal, c.tiempo_cosecha, gr.sugerencia_siguiente,
                ...
                WHERE p.nombre LIKE ? OR p.variedad LIKE ?
                ORDER BY $columna $direccion LIMIT ?, ?";

$stmt_datos = $conexion->prepare($query_datos);
$stmt_datos->bind_param("ssii", $busqueda_param, $busqueda_param, $inicio, $por_pagina);
$stmt_datos->execute();
...
<style>
    ...
    .sugerencia-box { font-size: 0.75rem; line-height: 1.1; color: #155724; }
</style>
...
<div class="container mt-4">
    ...
    <?php if (isset($_GET['status'])): ?>
        <div class="alert alert-success alert-dismissible fade show border-0 shadow-sm mb-4" role="alert" style="color: #000 !important;">
            ...
            <span class="fw-bold">
                <?php 
                    // Blindaje simple para evitar imprimir basura de la URL
                    $st = $_GET['status'];
    ...
    <div class="card mb-3 border-0 shadow-sm rounded-3">
        <div class="card-body p-3 d-flex flex-wrap justify-content-between align-items-center gap-3">
            <form class="d-flex gap-2 flex-grow-1" style="max-width: 500px;" method="GET">
                <input type="text" name="q" class="form-control form-control-sm shadow-sm" placeholder="Buscar planta o variedad..." value="<?= htmlspecialchars($buscar) ?>">
            ...
            <div class="table-responsive">
                <table class="table align-middle mb-0 table-hover">
                    <thead class="table-dark small">
                        <tr>
                            <th class="ps-3">
                                <a href="?orden=nombre&dir=<?= $siguiente_dir ?>&q=<?= urlencode($buscar) ?>" class="text-white text-decoration-none">
                                    NOMBRE <?= ($columna == 'nombre') ? ($direccion == 'ASC' ? '↑' : '↓') : '⇅' ?>
                                </a>
                            </th>
                            <th>
                                <a href="?orden=variedad&dir=<?= $siguiente_dir ?>&q=<?= urlencode($buscar) ?>" class="text-white text-decoration-none">
                                    VARIEDAD <?= ($columna == 'variedad') ? ($direccion == 'ASC' ? '↑' : '↓') : '⇅' ?>
                                </a>
                            </th>
                            <th>ÉPOCA / TIPO</th>
                            <th>
                                <a href="?orden=nombre_bancal&dir=<?= $siguiente_dir ?>&q=<?= urlencode($buscar) ?>" class="text-white text-decoration-none">
                                    UBICACIÓN <?= ($columna == 'nombre_bancal') ? ($direccion == 'ASC' ? '↑' : '↓') : '⇅' ?>
                                </a>
                            </th>
                            <th>ROTACIÓN SUGERIDA</th> <th class="text-center pe-3">ACCIONES</th>
                        </tr>
                    </thead>
                    <tbody class="small">
                        <?php while($row = $datos->fetch_assoc()): 
                            ...
                            if ($t_estimado > 0) {
                                if ($dias_vida > ($t_estimado + 15)) {
                                    $color_semaforo = "text-purple"; 
                                    $msg_cosecha = "Pasada / Floración / Semilla";
                                } elseif ($dias_vida >= $t_estimado) {
                                    $color_semaforo = "text-success";
                                    $msg_cosecha = "¡Lista para cosechar!";
                                } elseif ($dias_vida >= ($t_estimado * 0.7)) {
                                    $color_semaforo = "text-warning";
                                    $msg_cosecha = "Cosecha próxima";
                                }
                            }
                        ...
                        <tr>
                            <td class="ps-3">
                                <div class="d-flex align-items-center">
                                    ...
                                    <i class="bi bi-droplet-fill <?= $alerta_riego ? 'text-danger' : 'text-info' ?>" 
                                    title="Último riego: <?= htmlspecialchars($row['ultimo_riego'] ?? 'Nunca') ?>"></i>
                                </div>
                            ...
                            <td>
                                <div><?= htmlspecialchars($row['nombre_epoca'] ?: '-') ?></div>
                                <span class="badge bg-light text-dark border rounded-pill" style="font-size: 0.65rem;"><?= htmlspecialchars($row['nombre_tipo'] ?? 'General') ?></span>
                            </td>
                            <td>
                            ...
                            <td>
                                <div class="sugerencia-box">
                                    ...
                                    <?php else: ?>
                                        <span class="text-muted small">Sin datos de rotación</span>
                                    <?php endif; ?>
                                </div>
                            </td>
                            <td class="text-center pe-3">
                                <div class="btn-group shadow-sm">
                                    <a href="seguimiento.php?id=<?= (int)$row['id'] ?>" class="btn btn-sm btn-outline-primary" title="Ver Seguimiento">
                                        <i class="bi bi-eye"></i>
                                    </a>

                                    <a href="editar_planta.php?id=<?= (int)$row['id'] ?>" class="btn btn-sm btn-outline-warning" title="Editar">
                                        <i class="bi bi-pencil"></i>
                                    </a>

                                    <button onclick="confirmarBorrado(<?= (int)$row['id'] ?>)" class="btn btn-sm btn-outline-danger" title="Eliminar">
